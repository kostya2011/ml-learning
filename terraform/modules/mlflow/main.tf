...
resource "aws_apprunner_service" "mlflow_server" {
    ...
    image_repository {
      ...
      image_configuration {
        ...
        runtime_environment_variables = {
          "MLFLOW_ARTIFACT_URI"              = "s3://${aws_s3_bucket.main.id}"
          "MLFLOW_DB_DIALECT"                = "postgresql"
          "MLFLOW_DB_USERNAME"               = "${aws_db_instance.main.username}"
          "MLFLOW_DB_PASSWORD"               = "${random_password.main.result}"
          "MLFLOW_DB_HOST"                   = "${aws_db_instance.main.address}"
          "MLFLOW_DB_PORT"                   = "${aws_db_instance.main.port}"
          "MLFLOW_DB_DATABASE"               = "${aws_db_instance.main.db_name}"
  ...
  instance_configuration {
    ...
    instance_role_arn = aws_iam_role.main.arn
  }
